...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val bReload: Button = findViewById(R.id.reload)
        val bBackspace: Button = findViewById(R.id.backspace)
        val bExit: Button = findViewById(R.id.exitButton)
        ...
        bReload.setOnClickListener { onReload() }
        bBackspace.setOnClickListener { onBackspace() }
        bExit.setOnClickListener{ exitApp()}
    ...
    private fun onBackspace() {
        var calculatorText: String = displayTextView.text.toString()
        if (calculatorText != "") {
        if (!lastDot && !lastNum) {

            operator = null
            calculatorText = calculatorText.substring(0, calculatorText.length - 3)
            displayTextView.text = calculatorText
        } else if (lastDot) {
